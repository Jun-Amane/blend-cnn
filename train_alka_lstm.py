...
from dataset_text import AlkaDataset
...
# TODO: num_classes
num_classes = 102
net_obj = AlkaLSTM(hidden_dim=256, tagset_size=102, vocab_size=alka_set.vocab_size).to(training_device)
...
for i in range(epoch):
    ...
    with torch.no_grad():
        for captions, masks, labels in val_loader:
            captions = captions.to(training_device)
            masks = masks.to(training_device)
            labels = labels.to(training_device)
            outputs = net_obj(captions, masks)

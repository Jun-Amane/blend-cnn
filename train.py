...
from exp import *
from nts_net import *
from nts_net.model import MultiModalNTSNetModel
from dataset import AlkaDataset
from dataset_bert import AlkaDatasetBert
...
def build_dataset_bert(batch_size: int, size=300):
    data_tf = v2.Compose([
        v2.Resize((320, 320), interpolation=InterpolationMode.BICUBIC),
        v2.CenterCrop((size, size)),
        v2.RandomHorizontalFlip(),
        ...
        v2.ToDtype(torch.float32, scale=True),
        v2.Normalize((0.4355, 0.3777, 0.2879), (0.2653, 0.2124, 0.2194))])
    alka_set = AlkaDatasetBert('../dataset/102flowers', transform=data_tf, load_to_ram=False)
    train_ratio = 0.8
    dataset_size = len(alka_set)
    train_size = int(train_ratio * dataset_size)
    test_size = dataset_size - train_size
...
def build_m_cnn(word2idx, max_len):
    DROPOUT_1 = 0.2
    DROPOUT_2 = 0.4
    embeddings = load_pretrained_vectors(word2idx, "../data/crawl-300d-2M.vec")
    embeddings = torch.tensor(embeddings)
    num_classes = 102
    net_obj = ModifiedmCNN(num_filters=256, filter_sizes=[2,3,4], max_sequence_length=max_len, embedding_matrix=embeddings, embedding_dim=300, drop_out=DROPOUT_1, drop_out2=DROPOUT_2).to(
        training_device)
...
def build_inception_bert():
    num_classes = 102
    net_obj = GalloEtAls()

    pytorch_total_params = sum(p.numel() for p in net_obj.parameters())
    print(f"TOTAL PARAMS OF Inception-BERT: {pytorch_total_params}")
...
def build_nts_bert():
    num_classes = 102
    net_obj = MultiModalNTSNetModel()

    pytorch_total_params = sum(p.numel() for p in net_obj.parameters())
    print(f"TOTAL PARAMS OF NTS-BERT: {pytorch_total_params}")
...
def train_epoch_attn(model, train_loader, criterion, optimiser, cur_epoch: int):
    model.train()
    tqdm_epoch = tqdm(train_loader)
    for images, captions, attention_mask, labels in tqdm_epoch:
        images = images.to(training_device)
        labels = labels.to(training_device)
        captions = captions.squeeze(1).to(training_device)
        attention_mask = attention_mask.to(training_device)
...
def val_epoch_attn(model, val_loader, criterion, cur_epoch: int):
    total_step_loss = 0.0
    steps_per_epoch = 0
    total_accuracy = 0.0
    total_top5_accuracy = 0.0
...
def train_nts_bert():
    wandb.init(
        ...
        config={
            "model": "multimodal nts-net-bert",
            "learning_rate": 3e-4,
            "weight_decay": 1e-4,
            "batch_size": 16,
            "dataset": "Flowers 102 with BERT Tokenizer",
            "epochs": 30,
        })

    alka_set, train_loader, val_loader = build_dataset_bert(batch_size=wandb.config.batch_size, size=448)
    net_obj, para = build_nts_bert()
    ...
    for i in range(epoch):
        train_epoch_attn(net_obj, train_loader, criterion, optimiser, i + 1)
        acc = val_epoch_attn(net_obj, val_loader, criterion, i + 1)
        if acc > best_acc:
            best_acc = acc
...
def train_modified_mCNN():
    wandb.init(
        # Set the project where this run will be logged
        project="alka",
        # Track hyperparameters and run metadata
        config={
            "model": "modified m-CNN",
            "learning_rate": 3e-4,
            "weight_decay": 1e-4,
            "batch_size": 16,
            "dataset": "Flowers 102 Alka",
            "epochs": 30,
        })

    alka_set, train_loader, val_loader = build_dataset(batch_size=wandb.config.batch_size, size=224)
    net_obj, para = build_m_cnn(alka_set.word2idx, alka_set.max_len)
    criterion = build_criterion()
    optimiser = build_optimizer(net_obj, wandb.config.learning_rate, wandb.config.weight_decay)
...
def train_inception_bert():
    wandb.init(
        # Set the project where this run will be logged
        project="alka",
        # Track hyperparameters and run metadata
        config={
            "model": "multimodal inception-bert",
            "learning_rate": 3e-4,
            "weight_decay": 1e-4,
            "batch_size": 16,
            "dataset": "Flowers 102 with BERT Tokenizer",
    ...
    alka_set, train_loader, val_loader = build_dataset_bert(batch_size=wandb.config.batch_size, size=299)
    net_obj, para = build_inception_bert()
    criterion = build_criterion()
    optimiser = build_optimizer(net_obj, wandb.config.learning_rate, wandb.config.weight_decay)
